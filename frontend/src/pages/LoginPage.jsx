...
function LoginPage() {
  ...
  const handleLogin = async (e) => {
    e.preventDefault(); // form submit pe page reload nahi hoga
    setError("");
    setLoading(true);
    ...
    try {
      ...
      // Backend se token + user info milega
      const { token, name, role } = res.data;

      // Redux mein save karo (authSlice automatically localStorage mein bhi save karega)
      dispatch(setCredentials({ token, name, email, role }));

      // Admin hai toh admin panel pe bhejo, warna home pe
      if (role === "ADMIN") {
        navigate("/admin");
      } else {
        navigate("/");
  ...
  // -----------------------------------------------
  // Handle Google login — redirect to Spring Boot OAuth2
  // -----------------------------------------------
  const handleGoogleLogin = () => {
    // Yeh Spring Boot ka OAuth2 endpoint hai
    // Browser yahan jaayega → Spring Boot Google pe redirect karega
    const backendUrl =
    ...
    <div className="min-h-screen bg-gray-900 flex items-center justify-center px-4">
      <div className="bg-gray-800 rounded-2xl p-8 w-full max-w-md border border-gray-700">
        ...
        <div className="text-center mb-8">
          ...
          <p className="text-gray-400">Login to your ShopApp account</p>
        </div>
